...
import { preloadTrialIntroAssets } from '../screens/onboarding/trialIntroAssets';
import { initMixpanel, logEvent } from '../services/mixpanel';
import { prefetchReviewEnabled } from '../services/remoteConfig';
import { useAppDispatch } from '../store/hooks';
import { initPurchases, setInReview } from '../store/purchasesSlice';
...
// Kick off Remote Config before splash UI effects — don't wait for ATT / bootstrap.
...
export function AppSplash({ children }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      // Remote Config first — resolve + Redux ASAP (prefetch often already in-flight).
      const reviewPromise = prefetchReviewEnabled().then(reviewEnabled => {
        if (!cancelled) {
          dispatch(setInReview(reviewEnabled));
          log.info('inReview set from Remote Config (early)', { reviewEnabled });
        }
      ...
      const [, reviewEnabled] = await Promise.all([
        dispatch(initPurchases(state.deviceId))
          .unwrap()
          .then(result => {
      ...
      // Re-assert — keep Redux + bootstrap snapshot in sync for initial route.
      dispatch(setInReview(reviewEnabled));
      const nextState: BootstrapState = { ...state, inReview: reviewEnabled };
      log.info('inReview ready for reveal', { reviewEnabled });

      log.success('Splash complete — revealing app', {
        deviceId: nextState.deviceId,
        onboardingComplete: nextState.onboardingComplete,
        inReview: nextState.inReview,
      });
      void logEvent('splash_complete', {
        content_source: nextState.contentSource,
        onboarding_complete: nextState.onboardingComplete,
        is_new_user: nextState.isNewUser,
        fallback_used: false,
        duration_ms: Date.now() - splashStartedAt,
        in_review: nextState.inReview,
      });
      ...
      const [deviceId, onboardingComplete, reviewEnabled] = await Promise.all([
        loadUserId(),
